...
def split_l_u(data_set, n_labels):
    ...
    u_set = {"images": np.concatenate(u_images, 0), "labels": np.concatenate(u_labels, 0)}
    return l_set, u_set
...
def _load_svhn():
    ...
    for split in ["train", "test", "extra"]:
        ...
        splits[split] = {"images": tv_data.data, "labels": tv_data.labels}
...
def _load_cifar10():
    ...
    for train in [True, False]:
        ...
        splits["train" if train else "test"] = {"images": tv_data.data, "labels": np.array(tv_data.targets)}
    return splits.values()
...
if __name__ == "__main__":
    ...
    if args.dataset == "svhn":
        ...
        data_avg, zca_decomp = get_zca_normalization_param(train_set["images"])
        train_set["images"] = zca_normalization(train_set["images"], data_avg, zca_decomp)
        test_set["images"] = zca_normalization(test_set["images"], data_avg, zca_decomp)
