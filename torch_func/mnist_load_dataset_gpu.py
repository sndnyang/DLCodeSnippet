...
def load_dataset(dir_path, size=0, valid_size=0, keep=False, args=None):
    ...
    return Data(train_l['images'], train_l['labels'].astype(np.int32), args), \
        Data(train_ul['images'], train_ul['labels'].astype(np.int32), args), \
        Data(test['images'], test['labels'].astype(np.int32), args)
